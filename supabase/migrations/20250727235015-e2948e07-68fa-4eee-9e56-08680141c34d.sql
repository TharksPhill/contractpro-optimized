-- Corrigir função para rateio por número de produtos, não por projeção de vendas
...
BEGIN
    ...
    -- Contar produtos ativos do usuário
    SELECT COUNT(*) INTO total_active_products
    FROM products 
    WHERE user_id = product_data.user_id AND is_active = true;
    
    -- Evitar divisão por zero
    IF total_active_products = 0 THEN
        total_active_products = 1;
    END IF;
    
    -- CORREÇÃO: Rateio igual entre todos os produtos ativos
    product_fixed_cost_allocation = total_fixed_costs / total_active_products;
    ...
    product_total_cost = (product_data.supplier_cost + 
                        ...
                        END + 
                        product_data.shipping_cost) * COALESCE(product_data.quantity, 1);
    ...
    product_net_profit = product_gross_profit - 
                        ...
                        product_tax_cost - 
                        (product_data.customer_shipping_cost * COALESCE(product_data.quantity, 1)) - 
                        product_boleto_cost;
...
-- Atualizar o trigger principal também
CREATE OR REPLACE FUNCTION public.update_product_profit_analysis()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
    -- Chama a função manual com os dados do produto
    PERFORM update_product_profit_analysis_manual(NEW);
    RETURN NEW;
END;
$function$;
