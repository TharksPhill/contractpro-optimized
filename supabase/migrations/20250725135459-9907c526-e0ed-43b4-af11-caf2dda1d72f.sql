-- Corrigir a função de atualização da análise de lucro dos produtos
DROP FUNCTION IF EXISTS public.update_product_profit_analysis();
...
BEGIN
    ...
    product_selling_price = (NEW.supplier_cost + 
                           CASE WHEN NEW.ipi_type = 'percentage' 
                                ...
                                ELSE NEW.ipi_value 
                           END + 
                           NEW.shipping_cost) * (1 + NEW.markup_percentage / 100);
...
CREATE TRIGGER update_product_analysis_trigger
