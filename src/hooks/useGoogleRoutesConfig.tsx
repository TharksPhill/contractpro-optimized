...
export const useGoogleRoutesConfig = () => {
  const [config, setConfig] = useState<GoogleRoutesConfig | null>(null);
  const queryClient = useQueryClient();
  const { toast } = useToast();

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['google-routes-config'],
    queryFn: async () => {
      console.log('🔍 Buscando configuração do Google Routes...');
      ...
      return data && data.length > 0 ? data[0] as GoogleRoutesConfig : null;
    }
  });
  ...
  const saveConfig = async (apiKey: string): Promise<boolean> => {
    try {
      ...
      if (!isValidGoogleRoutesApiKey(apiKey)) {
        toast({
          title: "API Key Inválida",
          description: "A API key deve começar com 'AIza' e ter pelo menos 25 caracteres.",
          variant: "destructive",
        });
        return false;
      }
      ...
      setConfig(newConfig as GoogleRoutesConfig);
      
      // Invalidate and reload queries
      await queryClient.invalidateQueries({ queryKey: ['google-routes-config'] });
      await refetch();
      
      toast({
        title: "Sucesso!",
        description: "Configuração da Google Routes API salva com sucesso.",
      });
  ...
  const isValidGoogleRoutesApiKey = (apiKey: string): boolean => {
    return apiKey && 
           apiKey.length >= 25 && 
  ...
  const hasValidConfig = !!(config?.api_key && isValidGoogleRoutesApiKey(config.api_key));
  ...
  console.log('🔧 Hook status:', {
    ...
    apiKeyValid: config?.api_key ? isValidGoogleRoutesApiKey(config.api_key) : false
  });
